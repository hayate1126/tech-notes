...
## 📡 データ取得戦略 (Data Fetching Strategy)

本プロジェクトでは、Next.js App Routerの特性を活かし、**「パフォーマンス・安全性・UX」**　を最大化するための3層のデータ取得戦略を採用しています。

### 1. 取得手法の使い分け

| カテゴリ | 手法 | 主な用途 | 役割 |
| :--- | :--- | :--- | :--- |
| **Server Layer (RSC)** | `async/await` fetch | 初期表示データ, SEOコンテンツ | JS削減、シークレット情報の保護 |
| **Interaction Layer (RCC)** | SWR / React Query | 検索・フィルタ, 無限スクロール | クライアントキャッシュ, 高速な再検証 |
| **Mutation Layer** | Server Actions | データの登録・更新・削除 | 型安全な通信, サーバーキャッシュの即時更新 |

### 2. コア・ポリシー

#### ✅ Fetch where you use it (取得の局所化)
Next.jsのRequest Dedupingを活用し、Propsバケツリレーを避け、データが必要なコンポーネント（Feature層）で直接fetchを行います。これにより、コンポーネントの独立性を高めています。

#### ✅ Zodによるスキーマバリデーション
外部APIやServer Actionsの入力値は「信頼できないもの」と定義しています。全てのレスポンスに対して **Zod** によるスキーマバリデーションを行い、ドメイン層へ不正なデータが侵入するのを防いでいます。
...
export const getUser = async (id: string) => {
  ...
  return userSchema.parse(await res.json()); // 境界線でガード
};
...
重いデータ取得がページ全体のレンダリングをブロックしないよう、`Suspense` 境界を適切に配置しています。これにより、準備ができたコンテンツから順次表示（Streaming）させ、LCP（Largest Contentful Paint）の改善を図っています。

#### ✅ サーバーキャッシュの意図的な制御

ビジネス要件に応じて `revalidatePath` や `revalidateTag` を使い分け、Server Actions 実行後のキャッシュパージを適切に管理しています。

* **Static Data:** 永続的なキャッシュ（ISR）
* **Dynamic Data:** 操作の都度 `revalidate` を行うオンデマンド更新
